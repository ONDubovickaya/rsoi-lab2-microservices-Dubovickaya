...
async def post_rental_finish(rentalUid: str) -> Response:
    try:
        ...
        if rental.status != 'IN_PROGRESS':
            return Response(status=403, content_type='application/json', response=json.dumps({'errors': ['error not in progres']}))

        rental.status = 'FINISHED'
        rental.save()

        return Response(status=204, content_type='application/json', response=json.dumps(rental.to_dict()))
