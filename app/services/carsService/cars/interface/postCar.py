...
async def post_car_order(carUid: str) -> Response:
    try:
        ...
        if car.availability is False:
            return Response(status=403, content_type='application/json', response=json.dumps({'errors': ['Car is booked']}))

        car.availability = False
        car.save()

        return Response(status=200, content_type='application/json', response=json.dumps(car.to_dict()))
    except:
