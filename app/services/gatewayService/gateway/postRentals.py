...
async def post_rentals() -> Response:
    ...
    response = post_data_from_service('http://' + os.environ['CARS_SERVICE_HOST'] + ':' + os.environ['CARS_SERVICE_PORT'] + '/api/v1/cars/' + body['carUid'] + '/order', timeout=5)
    ...
    response = post_data_from_service('http://' + os.environ['PAYMENT_SERVICE_HOST'] + ':' + os.environ['PAYMENT_SERVICE_PORT'] + '/api/v1/payment', timeout=5, data={'price': price})

    if response is None:
        response = delete_data_from_service('http://' + os.environ['CARS_SERVICE_HOST'] + ':' + os.environ['CARS_SERVICE_PORT'] + '/api/v1/cars/' + body['carUid'] + '/order', timeout=5)
    ...
    response = post_data_from_service('http://' + os.environ['RENTAL_SERVICE_HOST'] + ':' + os.environ['RENTAL_SERVICE_PORT'] + '/api/v1/rental', timeout=5, data=body, headers={'X-User-Name': request.headers['X-User-Name']})

    if response is None:
        response = delete_data_from_service('http://' + os.environ['CARS_SERVICE_HOST'] + ':' + os.environ['CARS_SERVICE_PORT'] + '/api/v1/cars/' + body['carUid'] + '/order', timeout=5)
        response = delete_data_from_service('http://' + os.environ['PAYMENT_SERVICE_HOST'] + ':' + os.environ['PAYMENT_SERVICE_PORT'] + '/api/v1/payment/' + body['paymentUid'], timeout=5)
    ...
    return Response(status=200, content_type='application/json', response=json.dumps(rental))
